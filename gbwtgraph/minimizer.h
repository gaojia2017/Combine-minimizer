...
public:
  ...
  //Key64(key_type key_value, size_t hash_value) : key(key_value), hash(hash_value){}
  ...
  // Hash of the key.
  size_t cal_hash() const { return wang_hash_64(this->key & KEY_MASK); }
  //size_t get_hash() const { return this->hash; }
  // Move the kmer forward, with c as the next character. Update the key, assuming that
  // it encodes the kmer in forward orientation.
  void forward(size_t k, unsigned char c, size_t& valid_chars)
  {
...
public:
  ...
  */
  struct minimizer_type
  {
    ...
    bool operator==(const minimizer_type& another) const
    {
      return (this->hash == another.hash) && (this->key == another.key && this->offset == another.offset && this->is_reverse == another.is_reverse);
    }
...
//------------------------------------------------------------------------------
  ...
  explicit MinimizerIndex(bool use_syncmers = false) :
    ...
    com_hash(this->header.capacity, 0)
  {
  ...
  void swap(MinimizerIndex& another)
  {
    ...
    this->com_hash.swap(another.com_hash);
  }
  ...
  MinimizerIndex& operator=(MinimizerIndex&& source)
  {
    if(&source != this)
    {
      ...
      this->com_hash = std::move(source.com_hash);
    }
    return *this;
  }
...
//------------------------------------------------------------------------------
  ...
  */
  struct CircularBuffer
  {
    ...
    //myadd
    void clear() { this->tail = this->head;}
    ...
    {
      //The candidate at offset j is removed when we reach offset j + w
      if(!(this->empty()) && this->front().offset + this->w <= pos) { this->head++; }
      size_t forward_hash = forward_key.cal_hash(), reverse_hash = reverse_key.cal_hash();
  ...
  //添加gap模式的minimizer
    std::vector<minimizer_type> minimizers_gap(std::string::const_iterator begin, std::string::const_iterator end, int gap) const
  {
    ...
    std::string::const_iterator win_start= begin; //当前窗口的起始位置
    ...
    while(iter != end)
    {
      ...
      ++iter;
      if(static_cast<size_t>(iter - win_start) >= this->k()) { start_pos++; }
      // We have a full window with a minimizer.
      if(static_cast<size_t>(iter - win_start) >= window_length && !buffer.empty())
      {
  ...
  //添加e+2模式的minimizer
    std::vector<minimizer_type> minimizers_e2(std::string::const_iterator begin, std::string::const_iterator end, int gap) const
  {
    if(this->uses_syncmers()) { return this->syncmers(begin, end); }
    std::vector<minimizer_type> result;
    size_t window_length = this->window_bp(), total_length = end - begin;
  ...
  /*
    添加gap模式
  */
  std::vector<minimizer_type> minimizers_gap(const std::string& str, int gap) const
  {
    return this->minimizers_gap(str.begin(), str.end(), gap);
  }
  std::vector<minimizer_type> minimizers_e2(const std::string& str, int gap) const
  {
    return this->minimizers_e2(str.begin(), str.end(), gap);
  }
 
 //combine _gap 模式， graph的处理
   //添加gap模式的minimizer
  //   std::vector<minimizer_type> minimizers_combine_gap(std::string::const_iterator begin, std::string::const_iterator end, int gap) const
  // {
  //   if(this->uses_syncmers()) { return this->syncmers(begin, end); }
  //   std::vector<minimizer_type> result;
  //   size_t window_length = this->window_bp(), total_length = end - begin;
  ...
  //   //     result[j].hash = combineHash(result[j].hash, result[j + 1].hash);
  //   // }
  //   std::sort(result.begin(), result.end());
  ...
  std::vector<minimizer_type> minimizers_combine_gap(std::string::const_iterator begin, std::string::const_iterator end, int gap) const
  {
    ...
    // // //相邻minimizer组合
    // for(int j = 0; j + 1 < result.size(); j += 1){
    //     result[j].hash = combineHash(result[j].hash, result[j + 1].hash);
    // }
    ...
    // return result;
     ...
     // //相邻minimizer组合
    std::vector<minimizer_type> combine_result;
    for(int j = 0; j + 1 < result.size(); j += 2){
        result[j].hash = combineHash(result[j].hash, result[j + 1].hash);
        combine_result.emplace_back(std::move(result[j]));
    }
    std::sort(combine_result.begin(), combine_result.end());
  ...
  /*
    添加gap模式
  */
  std::vector<minimizer_type> minimizers_combine_gap(const std::string& str, int gap) const
  {
    return this->minimizers_combine_gap(str.begin(), str.end(), gap);
  }
  ...
  */
 //传统模式， 将最后得到的minimizer 按照ab cd ef组合
  ...
  */
  std::vector<std::tuple<minimizer_type, size_t, size_t>> minimizer_regions(std::string::const_iterator begin, std::string::const_iterator end) const
  {
    ...
    for(int j = 0; j + 1 < result.size(); j++){
        std::get<0>(result[j]).hash = combineHash(std::get<0>(result[j]).hash, std::get<0>(result[j + 1]).hash);
    }
...
//------------------------------------------------------------------------------
//添加gap模式
 std::vector<std::tuple<minimizer_type, size_t, size_t>> minimizer_regions_gap(std::string::const_iterator begin, std::string::const_iterator end, int gap) const
  {
    std::vector<std::tuple<minimizer_type, size_t, size_t>> result;
    if(this->uses_syncmers())
    {
    ...
    while(iter != end)
    {
      ...
      // We have a full window.
      if(static_cast<size_t>(iter - win_start) >= window_length)
      {
        // Work out where the window we are minimizing in began
        size_t window_start = static_cast<size_t>(iter - begin) - window_length;
...
//e+2模式
//添加gap模式
 std::vector<std::tuple<minimizer_type, size_t, size_t>> minimizer_regions_e2(std::string::const_iterator begin, std::string::const_iterator end, int gap) const
  {
    std::vector<std::tuple<minimizer_type, size_t, size_t>> result;
    if(this->uses_syncmers())
    {
    ...
    for(auto& record : result)
    {
     ...
     //print the minimizer info
    //auto& minimizers = result;
    // for (int i = 0; i < minimizers.size(); i++)
    //     {   
    //         std::cerr <<std::get<0>(minimizers[i]).hash<< std::get<0>(minimizers[i]).key << " "<< std::get<0>(minimizers[i]).offset<< " "<< 
    //                     std::get<1>(minimizers[i])<< " "<< std::get<2>(minimizers[i])<<std::endl;
    //     }
     //相邻minimizer组合
    for(int j = 0; j + 1 < result.size(); j++){
        // std::cerr <<std::get<0>(minimizers[j]).hash<< std::get<0>(minimizers[j]).key << " "<< std::get<0>(minimizers[j]).offset<< " "<< 
        //                 std::get<1>(minimizers[j])<< " "<< std::get<2>(minimizers[j])<<std::endl;
        //   std::cerr <<std::get<0>(minimizers[j + 1]).hash<< std::get<0>(minimizers[j + 1]).key << " "<< std::get<0>(minimizers[j + 1]).offset<< " "<< 
        //                 std::get<1>(minimizers[j + 1])<< " "<< std::get<2>(minimizers[j + 1])<<std::endl;
        std::get<0>(result[j]).hash = combineHash(std::get<0>(result[j]).hash, std::get<0>(result[j + 1]).hash);
    }
    if(result.size() > 0)
      result.resize(result.size() - 1);
  ...
  /*
    ...
    and length fields empty.
  */
  std::vector<std::tuple<minimizer_type, size_t, size_t>> minimizer_regions_gap(const std::string& str, int gap) const
  {
    return this->minimizer_regions_gap(str.begin(), str.end(), gap);
  }
  std::vector<std::tuple<minimizer_type, size_t, size_t>> minimizer_regions_e2(const std::string& str, int gap) const
  {
    return this->minimizer_regions_e2(str.begin(), str.end(), gap);
  }

//-------------------------------------------------------------------------------
//添加gap-combine 模式
//read 的处理模式, 从1-gap 不同的起始点开始，每一个起始点组合一次.这种方法会导致每一次minimizer_regions_e2的序列起点不一样，offset错误，因此要额外增加offset
std::vector<std::tuple<minimizer_type, size_t, size_t>> minimizer_regions_combine_gap2(int gap, std::string::const_iterator begin, std::string::const_iterator end) const
{
  std::vector<std::tuple<minimizer_type, size_t, size_t>> result;
  for(int i = 0; i < gap; i++){
    std::vector<std::tuple<minimizer_type, size_t, size_t>> temp = minimizer_regions_e2(begin + i, end, gap);
    for(auto m : temp){
        result.emplace_back();
        result.back() = std::move(m);
        std::get<0>(result.back()).offset += i;
    }
  }
  sort(result.begin(), result.end(), [&](std::tuple<minimizer_type, size_t, size_t>& a, std::tuple<minimizer_type, size_t, size_t> & b) -> bool {
      minimizer_type & aa = std::get<0>(a);
      minimizer_type & bb = std::get<0>(b);

      return (aa.hash < bb.hash) || (aa.hash == bb.hash && aa.key < bb.key) || 
              (aa.hash == bb.hash && aa.key == bb.key && aa.offset < bb.offset) ||
              (aa.hash == bb.hash && aa.key == bb.key && aa.offset == bb.offset && aa.is_reverse > bb.is_reverse ) ||
              (aa == bb && std::get<1>(a) < std::get<1>(b));
  });
  result.resize(std::unique(result.begin(), result.end(), [](std::tuple<minimizer_type, size_t, size_t>& a, std::tuple<minimizer_type, size_t, size_t> & b) -> bool {
      minimizer_type & aa = std::get<0>(a);
      minimizer_type & bb = std::get<0>(b);
      return aa == bb;
  }) - result.begin());
...
//也是组合+稀疏，不过是先把密集的minimizer的所有窗口都保存，然后双重循环求组合；
std::vector<std::tuple<minimizer_type, size_t, size_t>> minimizer_regions_combine_gap(int gap, std::string::const_iterator begin, std::string::const_iterator end) const
  {
    std::vector<std::tuple<minimizer_type, size_t, size_t>> result;
    if(this->uses_syncmers())
    {
    ...
    while(iter != end)
    {
      ...
      if(static_cast<size_t>(iter - begin) >= window_length)
      {
        ...
        if (!buffer.empty())
        {
            ...
            result.emplace_back(buffer.back(), window_start, 0);
            // If new minimizers beat out old ones, finish off the old ones.
            // while(!result.empty() &&
            //   finished_through < result.size() &&
            //   std::get<0>(result.back()).hash != std::get<0>(result[finished_through]).hash)
    ...
    //                     std::get<1>(minimizers[i])<< " "<< std::get<2>(minimizers[i])<<std::endl;
    //     }
    //     std::cerr<<"after combine---\n";
    // // //跳跃组合
    for(int i = 0; i < gap; i++){
        for(int j = i; j + gap < result.size(); j += gap){
          // std::cerr <<std::get<0>(minimizers[j]).hash<< std::get<0>(minimizers[j]).key << " "<< std::get<0>(minimizers[j]).offset<< " "<< 
          //               std::get<1>(minimizers[j])<< " "<< std::get<2>(minimizers[j])<<std::endl;
          // std::cerr <<std::get<0>(minimizers[j + gap]).hash<< std::get<0>(minimizers[j + gap]).key << " "<< std::get<0>(minimizers[j + gap]).offset<< " "<< 
          //               std::get<1>(minimizers[j + gap])<< " "<< std::get<2>(minimizers[j + gap])<<std::endl;
          std::get<0>(result[j]).hash = combineHash(std::get<0>(result[j]).hash, std::get<0>(result[j + gap]).hash);
        }
    }
    std::sort(result.begin(), result.end());
  ...
  /*
    ...
    and length fields empty.
  */
  std::vector<std::tuple<minimizer_type, size_t, size_t>> minimizer_regions_combine_gap(const std::string& str, int gap) const
  {
    return this->minimizer_regions_combine_gap2(gap,str.begin(), str.end());
  }
  ...
  std::vector<minimizer_type> syncmers(std::string::const_iterator begin, std::string::const_iterator end) const
  {
    ...
    while(iter != end)
    {
      ...
      if(valid_chars >= this->k())
      {
        ...
        {
          size_t forward_hash = forward_kmer.cal_hash(), reverse_hash = reverse_kmer.cal_hash();
...
//------------------------------------------------------------------------------
  ...
  */
  void insert(const minimizer_type& minimizer, const pos_t& pos, payload_type payload = DEFAULT_PAYLOAD)
  {
    ...
    {
      // if( minimizer.hash == 14967316640526472684){
      //     std::cerr<<"debug insert: key and pos "<<minimizer.key<<" "<<std:;get<0>(pos)<<std::endl;
      // }
      this->insert(minimizer.key, { code, payload }, offset);
      this->com_hash[offset] = minimizer.hash;
    }
    else if(this->hash_table[offset].first == minimizer.key)
    {
      // if( minimizer.hash == 14967316640526472684){
      //     std::cerr<<"debug append: key and pos "<<minimizer.key<<" "<<std:;get<0>(pos)<<std::endl;
      // }
  ...
  */
    ...
    std::pair<size_t, const hit_type*> count_and_find(const minimizer_type& minimizer) const
  {
    ...
    if(this->hash_table[offset].first == minimizer.key)
    {
      ...
      {
        // if(minimizer.hash == 14967316640526472684)
        //   std::cerr<<"hit.first = 1 "<<std::endl;
        result.first = 1; result.second = &(cell.second.value);
      }
    }
    return result;
    //std::cerr<<"if(this->hash_table[offset].first != minimizer.key"<<std::endl;
  }
...
public :
  void PrintIndexInfo(){
      std::cerr<<"header.key: "<<header.keys<<std::endl;
      std::cerr<<"header.value: "<<header.values<<std::endl;
  }

private:
  MinimizerHeader        header;
  std::vector<cell_type> hash_table;
  std::vector<size_t>    com_hash;
...
private:
 ...
 void insert(key_type key, hit_type hit, size_t offset)
  ...
  void rehash()
  {
    ...
    std::vector<size_t> old_com_hash(2 * this->capacity(), 0);
    this->com_hash.swap(old_com_hash);
    ...
    for(size_t i = 0; i < old_hash_table.size(); i++)
    {
      ...
      size_t hash = old_com_hash[i];
      if(source.first == key_type::no_key()) { continue; }

      size_t offset = this->find_offset(source.first, hash);
        // if(source.first.hash == 14967316640526472684)
        //   std::cerr<<"the offset is "<<offset<<std::endl;
      this->hash_table[offset] = source;
      this->com_hash[offset] = hash;
    }
  }
};
